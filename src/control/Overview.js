...
var ol_control_Overview = function (options) {
    ...
    // TODO: Extract bounceFn and elasticFn to utils class
    /** Elastic bounce
    ...
    this.ovmap_.addInteraction(new PointerInteraction(
        {
            handleDownEvent: function (evt) {	//TODO: Old version OL3
                if (ol.animation) {
                    var pan;
...
inherits(ol_control_Overview, Control);
...
ol_control_Overview.prototype.calcExtent_ = function (extent) {
    ...
    if (dx / res2 > 5 || dy / res2 > 5) {
        ...
        f.setGeometry(new Polygon([extent]));
    }
    else {
        f.setGeometry(new Point(center));
    }
    source.addFeature(f);
}
