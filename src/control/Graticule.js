 ...
 *    - style {Style} Style to use for drawing the graticule, default black.
...
var ol_control_Graticule = function (options) {
    ...
    var p = new Projection({code: this.get('projection')});
    ...
    if (options.style instanceof Style) this.style = options.style;
...
inherits(ol_control_Graticule, Control);
...
ol_control_Graticule.prototype.drawGraticule_ = function (e) {
    ...
    for (var i = 0, c; c = bbox[i]; i++) {
        bbox[i] = transform(c, map.getView().getProjection(), proj);
    ...
    for (var x = xmin; x < xmax; x += step) {
        var p0 = transform([x, ymin], proj, map.getView().getProjection());
        p0 = map.getPixelFromCoordinate(p0);
        if (hasLines) ctx.moveTo(p0[0], p0[1]);
        var p = p0;
        for (var y = ymin + step; y <= ymax; y += step) {
            var p1 = transform([x, y], proj, map.getView().getProjection());
    ...
    for (var y = ymin; y < ymax; y += step) {
        var p0 = transform([xmin, y], proj, map.getView().getProjection());
        p0 = map.getPixelFromCoordinate(p0);
        if (hasLines) ctx.moveTo(p0[0], p0[1]);
        var p = p0;
